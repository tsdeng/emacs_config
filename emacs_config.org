...
Manually maintained lisp files are in the ~/.emacs.d/lisp folder.
...
Making sure fonts in the code block is proper highlighted; Disable confirmation when executing a code block when =C-c C-c=
#+BEGIN_SRC emacs-lisp
...
Following settings are for all emacs built in functions, not from external plugins/modules
** Terminal Mouse
#+BEGIN_SRC emacs-lisp
(xterm-mouse-mode 1)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ;; Following code are deprecated, since osc52 seems to be a more portable and
  ;; universal solution. osc52 use escape code to send content to paste
  ;; board. Following terminals support osc52:
  ;; Linux -> Chrome's Secure Shell
  ;; OSX -> iTerm or Chrome's Secure Shell

  ;; Only in very special cases where you want to copy to the system clipboard
  ;; directly, call following function
  ;; (xclip-mode 1)
  ;; (osc-clipboard-mode +1)
...
#+END_SRC
** Click link behavior
You don't always want to open the link when click on it, especially when in a
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; browse-url-broser-function to cut the link into pasteboard, instead of
...
** No tabs
tabs makes things hard when different ppl have different tab-width
#+BEGIN_SRC emacs-lisp
...
** Repeat command
Press C-s ooo... to jump between windows
#+BEGIN_SRC emacs-lisp
  (check-install 'repeat
                 (lambda()
                   (require 'repeat)
      ...
      https://groups.google.com/forum/?hl=en&fromgroups=#!topic/gnu.emacs.help/RHKP2gjx7I8"
                   ...
                   (global-set-key (kbd "C-c <left>") (make-repeatable-command 'winner-undo))
                   ))
#+END_SRC
...
* Helm
#+BEGIN_SRC emacs-lisp
  (check-install 'helm (lambda()
                  ;;(require 'helm-config)
                  ;; try to use helm's autoload to reduce startup time
                  (helm-mode 1) ;; This function is costly, but damn... helm is too good
                  (global-set-key (kbd "C-x C-f")  'helm-find-files)
                  (global-set-key (kbd "M-s") 'helm-swoop)
                  (global-set-key (kbd "M-x") 'helm-M-x)

                  (defun auto-helm-projectile ()
                    "enbale projectile on demand"
                    (interactive)
                    (projectile-mode t)
                    (helm-projectile)
                    )
                  (global-set-key (kbd "C-c h p") 'auto-helm-projectile)
